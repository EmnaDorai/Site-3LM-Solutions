...
export default function AssistantPanel({
  besoins, synthese, generating, error, onGenerate, onUpdateBesoins, readOnly = false,
}: AssistantPanelProps) {
  const [instructions, setInstructions] = useState('');
  const [showBesoinsEdit, setShowBesoinsEdit] = useState(false);
  const [editedBesoins, setEditedBesoins] = useState(besoins);

  const handleGenerate = () => { onGenerate(instructions); setInstructions(''); };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 assistant-scroll">
        <div className="flex gap-3">
          <div className="w-7 h-7 shrink-0 rounded-full bg-[var(--line)] flex items-center justify-center text-[10px] font-mono-num uppercase">V</div>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] uppercase tracking-wider text-[var(--ink-soft)] font-mono-num mb-1.5">Besoins client</p>
            {showBesoinsEdit && onUpdateBesoins ? (
              <div className="space-y-2">
                <textarea value={editedBesoins} onChange={(e) => setEditedBesoins(e.target.value)} rows={5}
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => { onUpdateBesoins(editedBesoins); setShowBesoinsEdit(false); }}
                    className="text-xs text-white px-3 py-1.5 rounded-lg font-medium"
                    style={{ backgroundImage: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' }}>
                    Enregistrer
                  </button>
                  <button type="button" onClick={() => { setEditedBesoins(besoins); setShowBesoinsEdit(false); }}
              ...
              <div className="rounded-2xl rounded-tl-sm bg-[var(--surface)] border border-[var(--line)] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap">
                ...
                {!readOnly && onUpdateBesoins && (
                  <button type="button" onClick={() => setShowBesoinsEdit(true)}
                    className="block mt-2 text-xs font-medium hover:underline" style={{ color: 'var(--accent-primary)' }}>
                    Modifier
                  </button>
                )}
              </div>
        ...
        {generating ? (
          ...
          <div className="flex gap-3">
            <div className="w-7 h-7 shrink-0 rounded-full text-white flex items-center justify-center text-[10px] font-mono-num"
              style={{ backgroundImage: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' }}>IA</div>
            <div className="flex-1 min-w-0">
              <p className="text-[11px] uppercase tracking-wider text-[var(--ink-soft)] font-mono-num mb-1.5">Synthèse générée</p>
              <div className="rounded-2xl rounded-tl-sm px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap"
                style={{ background: 'linear-gradient(135deg, rgba(255,107,74,0.06), rgba(67,97,238,0.06))', border: '1px solid rgba(67,97,238,0.2)' }}>
                {synthese}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-dashed border-[var(--line)] p-6 text-center bg-[var(--surface)]/50">
            <p className="text-sm text-[var(--ink-soft)] mb-4">Aucune synthèse générée. Lancez l&apos;assistant pour créer le devis.</p>
            {!readOnly && (
        ...
        {error && (
          <div className="rounded-lg bg-[var(--accent-brick)]/10 border border-[var(--accent-brick)]/30 px-4 py-3 text-sm text-[var(--accent-brick)]">{error}</div>
        )}
      </div>
        ...
        <div className="border-t border-[var(--line)] p-5 bg-[var(--surface)] space-y-3">
          {hasSynthese && (
            <div className="flex flex-wrap gap-2">
              {PROMPT_SUGGESTIONS.map((s) => (
                <button key={s} type="button" onClick={() => setInstructions(s)}
          ...
          <div className="flex gap-2">
            <input type="text" value={instructions} onChange={(e) => setInstructions(e.target.value)}
              ...
              placeholder={hasSynthese ? 'Affinez le devis (ex. ajouter une ligne...)' : "Instructions optionnelles pour l'IA..."}
              disabled={generating}
            ...
            <button type="button" onClick={handleGenerate} disabled={generating}
              ...
              style={{ backgroundImage: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' }}>
              {generating ? 'Génération...' : hasSynthese ? 'Regénérer' : (
                <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" /></svg>Générer</>
              )}
            </button>
          </div>
        </div>
