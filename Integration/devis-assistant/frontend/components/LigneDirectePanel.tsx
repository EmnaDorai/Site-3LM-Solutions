...
export default function LigneDirectePanel({ devisId, clientId }: LigneDirectePanelProps) {
  ...
  const handleConfirmer = async (id: number) => {
    ...
    try {
      const { rendezVous: updated, emailError } = await confirmerRendezVous(id);
      setRendezVous((list) => (list ? list.map((r) => (r.id === id ? updated : r)) : list));
      setConfirmMessage(emailError ? 'Confirmé, mais l\'email n\'a pas pu être envoyé.' : 'Rendez-vous confirmé, email envoyé au client.');
    } catch {
      setConfirmMessage('Impossible de confirmer ce rendez-vous.');
    } finally {
      setConfirming(false);
    }
  };
  ...
  return (
    <div className="border-b border-[var(--line)] bg-[var(--surface)]">
      <div className="px-8 py-3.5 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2.5">
          ...
          {dernier ? (
            <Link href={`/rendezvous/${dernier.id}`} className="flex items-center gap-2 text-sm hover:underline">
              ...
              <span
                className="text-[10px] uppercase tracking-wider font-mono-num px-2 py-0.5 rounded-full"
                style={
                  dernier.source === 'public'
                    ? { background: '#E8ECFB', color: 'var(--accent-secondary)' }
                    : { background: '#F1F1F3', color: 'var(--ink-soft)' }
                }
              >
                {dernier.source === 'public' ? 'Demande client' : "Proposé par l'équipe"}
              </span>
            </Link>
          ) : (
        ...
        <div className="flex items-center gap-2 shrink-0">
          {dernier?.statut === 'demande' && (
            <button
              type="button"
              onClick={() => handleConfirmer(dernier.id)}
              disabled={confirming}
              className="text-xs font-semibold rounded-full px-4 py-2 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 text-white disabled:opacity-50"
              style={{ background: 'var(--accent-sage)' }}
            >
              {confirming ? 'Confirmation...' : '✓ Confirmer ce rendez-vous'}
            </button>
          )}
          <Link
      ...
      {confirmMessage && (
        <div className="px-8 pb-3 -mt-1 text-xs" style={{ color: 'var(--ink-soft)' }}>
          {confirmMessage}
        </div>
      )}
    </div>
